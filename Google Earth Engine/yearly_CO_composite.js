...
var coColl = dataset.select('CO_column_number_density');
...
var composite = years.map(function (y) {
  return coComp.filter(ee.Filter.eq('year', y)).median().set('year',y);
});
// print(composite, 'x');
...
var dependent = 'CO_column_number_density';
...
// Add variables.
var harmonicCO = decomposed_collection.map(
...
var fittedHarmonic = harmonicCO.map(function(image) {
...
var original_CO = decomposed_collection.select('CO_column_number_density');

//get fitted CO values
var fitted_CO = fittedHarmonic.select('fitted');

//combine bands
var combined = original_CO.combine(fitted_CO);

//fill gaps in original with fitted values
var gap_filled_composite = combined.map(function(img){
            return img.select('CO_column_number_density').unmask(img.select('fitted'));
});
...
Map.addLayer(singleBand,{bands: '2019_CO_column_number_density', min: 0, max: 0.5,
                       palette: ['blue', 'white', 'red']}, 'CO year');
...
batch.Download.ImageCollection.toAsset(gap_filled_composite, 'CO_Delhi', 
                {scale: 1000, 
                region: table,
                type: 'float'});
...
var batch = require('users/fitoprincipe/geetools:batch');
batch.Download.ImageCollection.toDrive(gap_filled_composite, 'CO_Delhi', 
                {scale: 1000, 
                region: table,
                type: 'float'});